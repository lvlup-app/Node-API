...
test('Creation fails if username or password is missing', async () => {
  ...
  await api
    ...
    .expect(400)
    .expect((res) => res.body.error.includes("username is required"))

  await api
    .post(url)
    .send(missingPassword)
    .expect(400)
    .expect((res) => res.body.error.includes("password is required"))
...
test('Creation fails if username or password is shorter than 3 characters', async () => {
  ...
  await api
    ...
    .send(shortUsername)
    .expect(400)
    .expect((res) => res.body.error.includes("is shorter than the minimum allowed"))
